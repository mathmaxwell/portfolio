...
import miniChat from '../images/miniChat.png'
...
const SkillList = ({
	...
	<ul className='flex gap-2 overflow-x-auto my-4'>
		{skills.map(skill => (
			<li
				key={skill}
...
const ProjectList = ({ projects }: IProject) => (
	<ul className='flex flex-col gap-6'>
		{projects.map((project, id) => (
			<li key={id} className='flex flex-col gap-2 border-t border-b py-4'>
				<a
					href={project.github}
				...
				<a href={project.site}>
					<img
						...
						className='max-w-full h-auto rounded-lg shadow-md hover:scale-105 transition-transform'
					/>
				</a>
				<ul className='flex flex-wrap gap-2'>
					{project.stack.map((stack, id) => (
						<li key={id} className='bg-gray-100 p-1 rounded'>
							{stack}
						</li>
					))}
				</ul>
...
const Skils = () => {
	const [mySkils, setMySkils] = useState([])

	const filteredProjects = myProjects.filter(project =>
		mySkils.every(skill => project.stack.includes(skill))
	)
	type HandleSkillClick = (skill: string) => void

	const handleSkillClick: HandleSkillClick = skill => {
		setMySkils(prev =>
	...
	return (
		<div className='container mx-auto px-4 sm:px-6 lg:px-8 my-10'>
			<h1 className='text-3xl sm:text-4xl lg:text-5xl text-center my-10 font-bold'>
				Skills
			</h1>
			<ul className='flex items-center justify-center gap-6 my-10'>
				<li>
					<svg
						width='35px'
						height='35px'
